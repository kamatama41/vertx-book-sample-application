...
MindMapEditor.prototype.renderVisualization = function() {
    ...
    nodes.forEach(function(d){
        d.y = d.depth * MindMapEditor.levelWidth;
    });
    var node = this.vis.selectAll("g.node").data(nodes, function(d){
        return d.key;
    });
    var nodeEnter = node.enter().append("svg:g")
        .attr("class", "node").attr("opacity", "0").attr("transform", function(d){
            return "translate(" + d.y + "," + d.x + ")";
        });
    nodeEnter.append("svg:circle").attr("r", 4.5).style("fill", "lightsteelblue").on("click", function(c){
        self.addNode(c);
    });
    nodeEnter.append("svg:text").attr("x", 10).attr("by", ".35em").text(function(d){
        return d.name;
    }).on("click", function(d){
    ...
    node.transition().attr("opacity", "1").attr("transform", function(d) {
        return "translate(" + d.y + "," + d.x + ")";
    ...
    link.enter().insert("svg:path", "g").attr("class", "link").attr("opacity", "0")
        .attr("d", MindMapEditor.diagonalGenerator).on("click", function(l){
            self.deleteNode(l.source, l.target);
        });
    link.transition().attr("d", MindMapEditor.diagonalGenerator).attr("opacity", "1");
    link.exit().remove();
};
